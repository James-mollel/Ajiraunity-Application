...
//     Globe, Link as LinkIcon, Download, Calendar, User 
// } from "lucide-react";

// export default function ViewApplicationDetails() {
//     const { applicationId } = useParams();

//     const [application, setApplication] = useState(null); // Changed to null for object check
...
//         try{
//             const resp = await api.patch(`jobs/details/application/status/${applicationId}/update/`, { status : newStatus })
...
//                 <Link to={-1} className="text-indigo-600 font-medium hover:underline">← Back to Applicants</Link>
               
//                 <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-200 mb-8">
//                 <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
//                     <div>
//                         <h2 className="text-sm font-bold text-gray-500 uppercase tracking-widest">Current Status</h2>
//                         <span className={`text-lg font-black ${
//                             application.status === 'REJECTED' ? 'text-red-600' : 'text-indigo-600'
//                         }`}>
...
//                         {/* Define the status flow buttons */}
//                         {[
//                             { label: "Pending", value: "PENDING", color: "bg-purple-600" },
//                             { label: "Under Review", value: "UNDER_REVIEW", color: "bg-purple-600" },
//                             { label: "Shortlist", value: "SHORTLISTED", color: "bg-purple-600" },
//                             { label: "Interview", value: "INTERVIEW", color: "bg-blue-600" },
...
//                                 className={`px-4 py-2 rounded-xl text-white text-sm font-bold transition-all hover:scale-105 active:scale-95 disabled:opacity-50 disabled:hover:scale-100 ${btn.color}`}
//                             >
//                                 {btn.label}
//                             </button>
//                         ))}
...
//                     <label htmlFor="">Applicants personam detail</label>
//                     <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6 text-center">
//                         {personal.avatar ? (
//                              <img 
//                             src={personal.avatar || "/default-avatar.png"} 
//                             alt="avatar" 
//                             className="w-32 h-32 rounded-2xl mx-auto object-cover border-4 border-indigo-50 shadow-sm mb-4"
//                         />
//                         ):(
//                             <div>
//                                 <User/>
...
//                         <h1 className="text-2xl font-bold text-gray-900">{personal.first_name} {personal.last_name}</h1>
//                         <p className="text-indigo-600 font-semibold mb-4">{info.job_title || info.title}</p>
                        
//                         <div className="space-y-3 text-left border-t border-gray-100 pt-6 mt-4">
//                             <div className="flex items-center gap-3 text-gray-600 text-sm">
//                                 <Mail size={18} className="text-gray-400" /> {personal.email}
//                             </div>
//                             <div className="flex items-center gap-3 text-gray-600 text-sm">
//                                 <Phone size={18} className="text-gray-400" /> {personal.phone_number || "No phone"}
//                             </div>
//                             <div className="flex items-center gap-3 text-gray-600 text-sm">
//                                 <MapPin size={18} className="text-gray-400" /> {personal.region}, {personal.district}, {personal.ward}
//                             </div>
//                         </div>

//                         <div className="bg-gray-50 p-4 rounded-xl border border-gray-100 mt-4">
//                             <p className="text-xs font-bold text-gray-400 uppercase">Expected Salary</p>
//                             <p className="text-xl font-black text-gray-900">
//                                 {info.currency} {Number(info.salary).toLocaleString()} 
//                                 <span className="text-sm text-gray-500 font-normal"> / {info.payment_period_dispaly}</span>
//                             </p>
//                             {application.job_post_type === "INDIVIDUAL" && (
//                                 <p className="text-sm text-indigo-600 font-bold mt-2">
//                                     Availability: {info.availability_display}
//                                 </p>
//                             )}
//                         </div>
...
//                             <label htmlFor="">Applicants social details</label>
//                              {info.linkedIn && <a href={info.linkedIn} target="_blank" className="p-2 bg-gray-50 rounded-lg hover:bg-indigo-50 text-gray-500 hover:text-indigo-600 transition-colors"><LinkIcon size={20} /> LinkedIn url</a>}
//                              {info.portfolio && <a href={info.portfolio} target="_blank" className="p-2 bg-gray-50 rounded-lg hover:bg-indigo-50 text-gray-500 hover:text-indigo-600 transition-colors"><LinkIcon size={20} />Portifolio url</a>}
//                             {info.gitHub && <a href={info.gitHub} target="_blank" className="p-2 bg-gray-50 rounded-lg hover:bg-indigo-50 text-gray-500 hover:text-indigo-600 transition-colors"><Globe size={20} />Github url</a>}
//                         </div>
...
//                         <div className="flex justify-center gap-4 mt-6">
//                            employment status {info.employment_status_dispaly || "-"}
//                             experience years {info.experience_yrs_dispaly || "-"}
//                            education lavel  {info.education_level_dispaly || "-"}
//                         </div>
...
//                         <h4 className="font-bold text-gray-900 mb-4 border-b pb-2">Application Info</h4>
//                         <div className="space-y-4">
//                             <div className="flex justify-between text-sm"><span className="text-gray-500">Applied On</span> <span className="font-medium">{new Date(application.applied_at).toLocaleDateString()}</span></div>
//                             <div className="flex justify-between text-sm"><span className="text-gray-500">Method</span> <span className="font-medium">{application.application_method_display}</span></div>
//                             <div className="flex justify-between text-sm"><span className="text-gray-500">Experience</span> <span className="font-medium">{info.experience_yrs || info.experience_level || "N/A"}</span></div>
//                            { application.job_post_type === "COMPANY"  && (
                         
//                            <div>
//                                 {application.cv && (
//                                 <a href={application.cv} download className="flex items-center justify-center gap-2 w-full mt-4 bg-gray-900 text-white py-3 rounded-xl font-bold hover:bg-indigo-600 transition-all">
...
//                 {/* RIGHT COLUMN: Detailed Resume Content */}
//                 <div className="lg:col-span-2 space-y-6">
                    
//                     {/* Cover Letter / Description */}
//                     <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-8">
//                         <h3 className="text-lg font-bold text-gray-900 mb-4 flex items-center gap-2">
//                             <User size={20} className="text-indigo-600" /> Cover Letter / About
//                         </h3>
//                         <p className="text-gray-600 leading-relaxed whitespace-pre-line">
//                             {application.cover_letter || info.description || "No description provided."}
//                         </p>
//                     </div>

//                     {/* EXPERIENCE SECTION (Conditional for Professionals) */}
//                 { application.job_post_type === "COMPANY"  && (
//                     <div>
...
//                                         <div className="absolute left-0 top-1.5 w-[24px] h-[24px] bg-white border-4 border-indigo-600 rounded-full shadow-sm"></div>
//                                         <h4 className="font-bold text-gray-900">{exp.role}</h4>
//                                         <p className="text-indigo-600 font-medium text-sm">{exp.company} • {exp.location}</p>
//                                         <p className="text-gray-400 text-xs mt-1 mb-2">{exp.start_date} — {exp.current_working ? "Present" : exp.end_date}</p>
//                                         <p className="text-gray-600 text-sm italic">{exp.responsibilities}</p>
//                                     </div>
//                                 ))}
//                             </div>
//                         </div>
...
//                                         <h4 className="font-bold text-gray-900 text-sm">{edu.degree || edu.education_level}</h4>
//                                         <p className="text-indigo-600 text-sm">{edu.school}</p>
//                                         <p className="text-gray-500 text-xs mt-1">{edu.field_of_study}</p>
//                                         <p className="text-gray-400 text-[10px] mt-2 uppercase font-bold tracking-widest">{edu.start_year} - {edu.end_year || 'Present'}</p>
//                                     </div>
//                                 ))}
//                             </div>
//                         </div>
...
//                             <div className="flex flex-wrap gap-2">
//                                 {info.skills.map((s, i) => (
//                                     <span key={i} className="px-4 py-2 bg-indigo-50 text-indigo-700 rounded-lg text-sm font-medium border border-indigo-100 uppercase tracking-tighter">
//                                         {s.skill}
//                                     </span>
//                                 ))}
//                             </div>
...
//                                     <div key={i}>
//                                         <span className="px-4 py-2 bg-indigo-50 text-indigo-700 rounded-lg text-sm font-medium border border-indigo-100 uppercase tracking-tighter">
//                                             {l.language}
//                                             {l.level}
//                                         </span>
//                                     </div>
