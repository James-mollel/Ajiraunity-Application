...
import { ArrowLeft, BookIcon, BookOpen,
        DollarSign, MailIcon, MapPin,
       UserCogIcon } from "lucide-react";


export default function EditProfessionalJob() {
    ...
    const fetchAJob = async()=>{
        try{
            ...
            setJobType(resp.data.job_type || "")
            setGender(resp.data.gender || "")
            setExperienceLevel(resp.data.experience_level || "")
            setEducationLevel(resp.data.education_level || "")

            setMaxSalary(resp.data.salary_max || "")
            setMinSalary(resp.data.salary_min || "")
            setSalaryvisible(resp.data.salary_visible)
            ...
            setSkillsRequired(resp.data.skills_required || "")

            setThroughtEmail(resp.data.apply_email || "")
            setThroughtYourWeb(resp.data.apply_website || "")
            setThroughtInapp(resp.data.apply_in_app)
            ...
            if (resp.data.company){
                setCompany(resp.data.company.id)
            }
    ...
    useEffect(()=>{
      const fetchInitData = async()=>{
         await   Promise.all( [ fetchProfessionalCategories(), fetchRegions(), fetchCompanies()]) 
      }
      fetchInitData();
    },[]); 
    ...
    return(
 <div className="sm:p-6 md:p-8 bg-gray-100 min-h-screen">
    <div className="max-w-5xl mx-auto bg-white shadow-2xl rounded-xl p-6 sm:p-8 lg:p-10">
        <div className="flex items-center mt-9 mb-4 md:mt-0 space-x-4">
            <ArrowLeft onClick={()=>navigate(-1)} className="cursor-pointer"/>
             <h2 className="text-xl md:text-2xl font-extrabold text-gray-900 border-b-4 border-indigo-600 pb-3">
              ✨ Edit Professional Job
            </h2>
        </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 items-center">
                    ...
                    <div className="md:col-span-1">
                        <label htmlFor="email_app" className="block text-sm font-semibold text-gray-700 mb-1">
                            Receive Applications Through Company email
                        </label>
                        <input
                            type="email"
                            id="email_app"
