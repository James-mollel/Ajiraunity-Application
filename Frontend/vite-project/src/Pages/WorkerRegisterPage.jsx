...
export default function WorkerRegistrationPage() {
    ...
    const handleSubmit = async(e) => {
        ...
        if (Object.keys(validationErrors).length === 0) {
            ...
            try{
                const resp = await api.post("user-authentications/signup/worker/user/", values);
                if (resp.status === 201){
        ...
        <div className="min-h-screen flex items-center justify-center bg-gray-50 py-12 px-4 sm:px-6 lg:px-8">
            <div className="max-w-md w-full space-y-8 p-10 bg-white shadow-xl rounded-sm border border-gray-200">
                <div className="text-center">
                    <h2 className="mt-6 text-3xl font-extrabold text-gray-900">Join as a <span className="text-indigo-400">Job Seeker</span></h2>
                    <p className="mt-2 text-sm text-gray-600">Find <span className="font-semibold">company</span> jobs or <span className="font-semibold">street</span> gigs matching your skills.</p>
                </div>
